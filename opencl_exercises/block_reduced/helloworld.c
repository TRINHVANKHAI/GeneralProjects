...
#define GLOBAL_WIDTH 256
#define GLOBAL_HEIGHT 256
...
uint16_t output_array[GLOBAL_WIDTH*GLOBAL_HEIGHT];
...
int main (int argc, char **argv)
{
    ...
    printf ("\nResult:\n");
    for(col=0;col<(256*256);col++)
    {
    	printf("%d, ", output_array[col]);
    }
    printf("\n");
    printf("Input array: \n");
    ...
    for (row = 0; row < buffer_height; row++)
    {
        ...
        {
            printf ("%d -- %d, ", input_array[row][col], output_array[row][col]);
        }
        printf("\n");
    }
    #endif
